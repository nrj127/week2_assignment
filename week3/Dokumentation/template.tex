...
%5IEEEtran.cls version 1.8 and later.
...
The Caffe framework \cite{} from UC Berkeley is designed to let researchers and novice learners create and explore convolutional neural networks  and other Deep Neural Networks easily.
Caffe provides state-of-the art modeling for advancing and deploying deep learning in research and industry with support for a wide variety of architectures and efficint implementations of prediction and learning.The Caffe framework follows a layered architecture, following a b
% \newpage


\subsection{Back Propogation Algorithm}
...
The figure below shows the 8 images that were to be classified using the ilsvrc2 2012 training model.
...
Probability of class : In a multi-output classification problem, the probability if a class plays a very import role. The probability of a class indicates the likelihood of occurrence of a particular class. Given a particular unknown input, the corresponding output vector O, then the estimated probabilty that it belongs to each class is given as follows.

\[P_{cO}(c \mid O) = \frac{p(c \mid O)}{\Sigma _{c^{'} } p(c^{l} \mid O )  }              \]

%\begin{center}
%\includegraphics{ex2_images/IMG_1.jpg}
%\end{center}
